...
router.get("/select", validatetoken, async function (request, response) {
  try {
...
router.get("/fetchlocation", validatetoken, async (req, res) => {
...
router.post("/create", validatetoken, async (req, res) => {
  ...
  var transaction_history = {
    ...
    timestamp: new Date(),
    desciption:
      " shipment created sucessfully from " +
      locationNames.from_location_name +
      " to " +
...
router.post("/addshipmentId", validatetoken, (req, res) => {
  ...
  aliquots_samples_key.forEach((element, index) => {
    var transaction_history = {
      ...
      timestamp: new Date(),
      desciption:
        " shipment id and status updated for aliquot having aliquots_samples_key " +
        element,
    };
    var query = db.query(
      "UPDATE `aliquots` SET `shipment_id` = ?,`status_id` = 2 where aliquots_samples_key=? limit ?",
      [req.body.shipment_id, element, countArray[index]],
      (error, results, fields) => {
        if (error) throw error;
        var query = db.query(
          "INSERT INTO `transaction_history` SET ?",
...
router.get("/filterOpts/", validatetoken, async (req, res) => {
  if (req.query.fromLocation === undefined) {
    ...
    var query = await db.query(
      `select distinct S.sample_id, S.eval, COUNT(*) as aliquot_count from samples S INNER JOIN aliquots A ON S.samples_key=A.aliquots_samples_key INNER JOIN locations L ON L.location_id = A.location_id where S.type!='' AND A.status_id!=2 AND L.location_name = ? GROUP BY S.sample_id, S.eval,S.type,L.location_name`,
      [fromLocation.value],
      (error, results, fields) => {
        if (error) throw error;
        // console.log(results);
...
router.get("/aliquots/:sample_id", validatetoken, async (req, res) => {
  var query = await db.query(
