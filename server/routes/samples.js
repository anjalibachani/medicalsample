  ...
  for (let index = 0; index < result.length; index++) {
    ...
    var select_query = db.query(select_stmt, [element.sample_id, element.eval], (error, select_results) => {
      ...
      var transaction_history = {
        ...
        timestamp: new Date(),
        desciption: " updated samples having ID " + element.sample_id+" eval "+element.eval+" type "+element.type,
      };
        ...
        var insert_query = db.query( insert_stmt,element,(error, results, fields) => {
          ...
          var transaction_history = {
            user_id: result[index].user_id,
            timestamp: new Date(),
            desciption: " created samples having ID " + element.sample_id+" eval "+element.eval+" type "+element.type,
          };
          var query = db.query("INSERT INTO `transaction_history` SET ?", transaction_history, (err, res) => {
            if (err) throw err;
            console.log(res.insertId);
          ...
          for (let i = 0; i < maxItr; i++) {
            ...
            var transaction_history = {
              user_id:result[index].user_id,
              timestamp: new Date(),
              desciption: " created aliquots at location id "+aliquot.location_id+" with aliquots_samples_key "+ aliquot.aliquots_samples_key,
            };
            if (error) throw error;
          var query = db.query("INSERT INTO `transaction_history` SET ?", transaction_history, (err, res) => {
            if (err) throw err;
