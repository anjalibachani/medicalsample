...
router.post("/markshipments", (req, res) => {
  ...
  for (let index = 0; index < rowsData.length; index++) {
    ...
    let update_query = db.query(update_stmt,[item[0]],(error, results, fields) => {
      ...
      db.query("INSERT INTO `transaction_history` SET ?", transaction_history, (err, res) => {
        if (err) throw err;
    ...
    let aliquot_update_query = db.query("UPDATE aliquots SET location_id = ?,`status_id` = 1 WHERE shipment_id = ?",[item[1],item[0]], (err, res) => {
      ...
      var transaction_history = {
        user_id: item[2],
        timestamp: new Date(),
        desciption: "location and status updated for aliquots having shipment_id " + item[0],
      };
      db.query("INSERT INTO `transaction_history` SET ?", transaction_history, (er, out) => {
        if (er) throw er;
